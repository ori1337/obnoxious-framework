	...
	if (Enable && CGlobal::IsGameReady && !CGlobal::FullUpdateCheck)
	{
		if (plocal)
		{
			...
			if (ClanTagChanger)
			{
				...
				{
				case 0: NewClan = XorStr("obnoxious > All"); break;
				...
				case 2:
				{
					...
					case 3: NewClan = XorStr("xD"); break;
			...
			if (ChatSpam && ChatSpamStart)
			{
				...
				{
				case 0: chatspam = { XorStr("obnoxious") }; break;
...
{
	SoundList.clear();
	string SoundsDir = CGlobal::SystemDisk + XorStr("obnoxious framework\\Resources\\Sounds\\*.wav");
		...
		if (GP_Misc->HitSound)
		{
			if (PlaySound)
			{
				if (!GP_Misc->SoundList.empty() && GP_Misc->HitSoundStyle >= 0 && GP_Misc->HitSoundStyle < (int)GP_Misc->SoundList.size())
					FastCall::G().t_PlaySoundA((CGlobal::SystemDisk + XorStr("obnoxious framework\\Resources\\Sounds\\") + GP_Misc->SoundList[GP_Misc->HitSoundStyle]).c_str(),
